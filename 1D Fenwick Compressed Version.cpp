...
struct Segment{
    ...
    int Query(int Idx, int Val1, int Val2){
        Idx++;
        int Res = 0;
        for( ; Idx; Idx -= Idx & (-Idx)){
            Res += Set[Idx].ook(Val2) - Set[Idx].ook(Val1);
        }
        return Res;
    }
}Seg;
...
public:
    void Solve(istream& cin, ostream& cout){
        ...
        while(m--){
            ...
            int a = Track[x];
            int b = Track[y];
            Res += (Seg.Query(y, a, b) << 1);
            Res -= (Seg.Query(x+1, a, b) << 1);

            if(Track[x] < Track[y]) Res++;
            else Res--;
            Swap(Track[x], Track[y]);
            Seg.Erase(x, a);
            Seg.Erase(y, b);
            Seg.Add(y, a);
            Seg.Add(x, b);
